	...
	<table border="1">
		...
		<tr>
			...
			<td>
				<select name="phone1">
					...
					<option value="010" <c:if test="${dto.phone1 == '010'}">selected</c:if>>010</option>
					<option value="011" <c:if test="${dto.phone1 == '011'}">selected</c:if>>011</option>
					<option value="016" <c:if test="${dto.phone1 == '016'}">selected</c:if>>016</option>
				</select>
		...
		<tr>
			...
			<td>
				...
				<select name="email2">
					<option value="">-- 선택 --</option>
					<option value="google.com" <c:if test="${dto.email2 == 'google.com'}">selected</c:if>>google.com</option>
					<option value="naver.com" <c:if test="${dto.email2 == 'naver.com'}">selected</c:if>>naver.com</option>
					<option value="daum.net" <c:if test="${dto.email2 == 'daum.com'}">selected</c:if>>daum.net</option>
				</select>
			</td>
		</tr>
		<tr>
			<td rowspan="3">주소</td>
			<td>
				<input type="text" id="sample6_postcode" placeholder="우편번호" name="postcode" readonly value="${dto.postcode }">
		...
		<tr>
			<td>
				...
				<input type="text" id="sample6_extraAddress" placeholder="참고항목" name="extraAddress" readonly value="${dto.extraAddress }">
			</td>
		</tr>
...
<script>
	...
	function sujung(){
		if(confirm('수정하시겠습니까?')){
			...
			f.action = "${path }/member/sujungProc";
			f.method = "post";
			f.submit();
		}
	}
